...
export class App extends Component {
    ...
    async componentDidMount() {
        ...
        let flightPuchased = this.airline.FlightPurchased();
        console.log(flightPuchased);
        flightPuchased.watch(function(err, result){
            console.log('entro?')
            const {customer, price, flight} = result.args;

            if(customer === this.state.account) {
                console.log(`you purchgase a flight to ${flight} with a cost of ${price}`);
            } else {
                console.log('dale capo')
            ...
            <div className="row">
                ...
                <div className="col-sm">
                    <Panel title="Loyalty points - refundable ether">
                        <span><strong></strong> {this.state.refundableEther } ether</span>
